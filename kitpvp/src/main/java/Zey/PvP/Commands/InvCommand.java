	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String Label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("inv")) {
			if (sender.hasPermission("zey.pvp.inv")) {
				sender.sendMessage(
						String.valueOf(Main.NAME) + " §7» §cErrado, utilize a sintaxe correta: /inv [Jogador(a)]");
				if (args.length > 0) {
					...
					sender.sendMessage(String.valueOf(Main.NAME) + " §7» §7Inventario §a§lABERTO§7 com sucesso.");

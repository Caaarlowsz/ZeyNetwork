	...
	public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("iniciar")) {
			...
			if (args[0].equalsIgnoreCase("evento")) {
				if (evento) {
					p.sendMessage(String.valueOf(Main.NAME) + " §7» §cO Evento [Evento] já foi iniciado.");
					return true;
				} else {
					p.sendMessage(String.valueOf(Main.NAME) + " §7» §aVocê iniciou o Evento [Evento].");
			...
			if (args[0].equalsIgnoreCase("fullkit")) {
				if (fullkit) {
					p.sendMessage(String.valueOf(Main.NAME) + " §7» §cO beneficio FullKit já está liberado.");
					return true;
				} else {
					p.sendMessage(String.valueOf(Main.NAME) + " §7» §aVocê liberou o beneficio FullKit para todos.");
					Bukkit.broadcastMessage(String.valueOf(Main.NAME)

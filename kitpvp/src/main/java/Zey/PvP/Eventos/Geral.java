...
public class Geral implements Listener {
	...
	public void onMe(final PlayerCommandPreprocessEvent event) {
		...
		if (event.getMessage().toLowerCase().startsWith("/me") || event.getMessage().toLowerCase().startsWith("//calc")
			...
			p.sendMessage(String.valueOf(Main.NAME) + " §7» §cComando não listado");
		}
	}
	...
	public void onDeath(final PlayerDeathEvent e) {
		...
		if (p.getKiller() instanceof Player) {
			...
			p.sendMessage(String.valueOf(Main.NAME) + " §7» §cVocê foi morto por " + k.getName());
			k.sendMessage(String.valueOf(Main.NAME) + " §7» §aVocê adquiriu 50 ZeyCoins!");
			p.sendMessage(String.valueOf(Main.NAME) + " §7» §cVocê perdeu 50 ZeyCoins!");
			k.sendMessage(String.valueOf(Main.NAME) + " §7» §aVocê matou o jogador(a): §e" + p.getName());
	...
	public void banAds(final PlayerChatEvent e) {
		...
		if (!p.hasPermission("zey.pvp.falarsites") && (e.getMessage().contains(".net")
				...
				|| (e.getMessage().contains(",net") || e.getMessage().contains(",host")
						...
						|| e.getMessage().contains(",cookiehosting") || e.getMessage().contains(",playmine")
						|| e.getMessage().contains(",com") || e.getMessage().contains(",server")
						|| e.getMessage().contains(",me") || e.getMessage().contains(",io")
						|| e.getMessage().contains(",com") || e.getMessage().contains(",com.br")
	...
	public void banofensa(final PlayerChatEvent e) {
		final Player p = e.getPlayer();
		if (!p.hasPermission("zey.pvp.falarmerda") && (e.getMessage().contains("filho da puta")
				|| e.getMessage().contains("vadia") || e.getMessage().contains("sua m§e")
				|| e.getMessage().contains("sua mae") || e.getMessage().contains("aql puta")
				|| e.getMessage().contains("seu lixo") || e.getMessage().contains("puta"))) {
			e.setCancelled(true);
			p.sendMessage(
		...
		if (e.getPlayer().getItemInHand().getType() == Material.COMPASS) {
			for (final Entity entidades : e.getPlayer().getNearbyEntities(100.0, 150.0, 100.0)) {
				if (entidades instanceof Player
					...
					e.getPlayer().sendMessage(String.valueOf(Main.NAME)
							+ " §7» Bússola apontando para o jogador(a): §e" + ((Player) entidades).getName());
					return;
				}
			}
			if (!parar) {
				e.getPlayer().sendMessage(String.valueOf(Main.NAME) + " §7» Nenhum jogador(a) foi encontrado!");
